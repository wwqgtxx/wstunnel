...
func (t *Tester[T]) SniffQuic(nextBlock []byte) (d string, err error) {
	isQuic := false
	for {
...
func sniffQuicBlock(buf []byte) (d string, next []byte, err error) {
	...
	firstByte := header[0]
	rawPacketNumber := make([]byte, quicutils.MaxPacketNumberLength)
	...
	plaintext, err := quicutils.DecryptQuic(header, blockEnd, destConnId)
	if err != nil {
		return "", nil, NotApplicableError
	}
...
func extractSniFromQuicPayload(payload []byte) (sni string, err error) {
	helloBytes := make([]byte, len(payload)+5)

	// One payload may have multiple frames.
	// Reassemble Crypto frames.
	b, err := quicutils.ReassembleCryptoToBytes(payload, helloBytes[5:])
	if err != nil {
		return "", err
	}

	copy(helloBytes, tls.StartBytes[:])
	binary.BigEndian.PutUint16(helloBytes[3:], uint16(len(b)))

	sni = tls.ExtractSniFromBytes(helloBytes[:len(b)+5])
	if len(sni) > 0 {
		return sni, nil
	}
	return "", NotFoundError
